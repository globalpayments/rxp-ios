...
open class HPPManager: NSObject, UIWebViewDelegate, HPPViewControllerDelegate {
	...
	*/
	...
	/**
	* The shared secret value for each merchant.
	*/
	...
	*/
	...
	fileprivate func getPayByLink() {
		...
		let cachePolicy = NSURLRequest.CachePolicy.reloadIgnoringLocalCacheData
		let request: NSMutableURLRequest = NSMutableURLRequest(url: HPPURL, cachePolicy: cachePolicy, timeoutInterval: 30.0)
		...
		var data = Data.init()
		do {
			data = try JSONSerialization.data(withJSONObject: getPaybylinkParameters(), options: .prettyPrinted)
		} catch let error {
			print(error)
		}
	...
	fileprivate func getPaybylinkParameters() -> Dictionary<String, String> {
		...
		parameters = ["MERCHANT_ID" : self.merchantId,
					  ...
					  "COMMENT2" : self.commentTwo,
					  "RETURN_TSS" : self.returnTss,
					  "SHIPPING_CODE" : self.shippingCode,
					  "SHIPPING_CO" : self.shippingCountry,
					  "BILLING_CODE" : self.billingCode,
					  "BILLING_CO" : self.billingCountry,
					  "CUST_NUM" : self.customerNumber,
					  "VAR_REF" : self.variableReference,
...
//Date Formatter
private func getDateAsString(format:String) -> String {
